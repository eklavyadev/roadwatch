...
export default function TenderDataPage() {
  ...
  // Fetch contract/tender data from the new tenders API
  const fetchContracts = async () => {
    ...
    try {
      ...
      console.error(err);
      setToast({ message: "Failed to fetch tender records from database.", type: 'error' });
  ...
  useEffect(() => {
    fetchContracts();
    setCurrentPage(1);
  }, [selectedCategory, selectedType, selectedYear, selectedState, searchQuery]);
        ...
        <div className="mx-auto max-w-7xl space-y-12 animate-[fadeIn_0.5s_ease-out]">
          ...
          <section className="text-center max-w-4xl mx-auto space-y-5">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-cyan-950/40 border border-cyan-800/30 text-cyan-400 text-xs font-semibold tracking-wider uppercase mb-1">
              ...
              State-Wise Civic & Highway Tender Registry
            </div>
            <h1 className="text-4xl sm:text-6xl font-bold tracking-tight leading-tight">
              Tender Data <span className="text-cyan-400">Ledger</span>
            </h1>
            <p className="text-slate-400 text-lg leading-relaxed max-w-2xl mx-auto">
              Segregated civic contract and highway spending details, fetched directly from state records. Filter by department, state, and year.
            </p>
          </section>
          ...
          <div className="bg-[#0f172a]/60 border border-slate-800 p-6 rounded-xl space-y-4 max-w-6xl mx-auto">
            <div className="grid grid-cols-1 md:grid-cols-12 gap-4 items-center">
              ...
              {/* Category Tab Switcher: NH vs State Projects */}
              <div className={`flex bg-[#020817] border border-slate-800 p-1.5 rounded-lg transition-all duration-300 ${
                ...
                <button
                  onClick={() => setSelectedCategory("StateProjects")}
                  className={`flex-1 text-center py-2.5 text-xs font-bold uppercase tracking-wider rounded-md transition duration-200 ${
                    selectedCategory === "StateProjects" ? 'bg-cyan-500 text-[#020817]' : 'text-slate-400 hover:text-white'
                  }`}
                >
                  State Projects
                </button>
              ...
              {/* State Filter Dropdown (State Projects Only) */}
              {selectedCategory === "StateProjects" && (
                <div className="md:col-span-3 relative animate-[fadeIn_0.2s_ease-out]">
                  <select
                    value={selectedState}
              ...
              {/* Type Filter Dropdown (State Projects Only: State Highways, PWD, Municipality) */}
              {selectedCategory === "StateProjects" && (
                <div className="md:col-span-3 relative animate-[fadeIn_0.2s_ease-out]">
                  <select
                    value={selectedType}
                    ...
                    className="w-full bg-[#020817] border border-cyan-500/30 text-xs font-bold uppercase tracking-wider py-3 px-4 rounded-lg focus:outline-none focus:border-cyan-500 text-slate-200 appearance-none cursor-pointer hover:border-cyan-500/60 transition duration-200"
                  >
                    <option value="StateProjects">All Projects</option>
                    <option value="SH">State Highways (SH)</option>
                    <option value="PWD">Public Works (PWD)</option>
                    <option value="Municipality">Municipality (Civic)</option>
                  </select>
                  <div className="absolute inset-y-0 right-3 flex items-center pointer-events-none text-cyan-400 text-xs font-semibold">
                    ▼
                  </div>
              ...
              {/* Year Filter Dropdown: Dynamic based on Category Selection */}
              <div className={`relative transition-all duration-300 ${
                selectedCategory === "StateProjects" ? 'md:col-span-2' : 'md:col-span-4'
              }`}>
                <select
                  value={selectedYear}
              ...
              <div className="md:col-span-12 relative mt-1">
                <input
                  type="text"
                  placeholder='Search by tender ID, description, local body, or contractor...'
                  value={searchQuery}
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6 max-w-6xl mx-auto">
            ...
            <StatCard
              title="Tender Contracts"
              value={aggregates.totalContracts}
              desc="Ingested government tenders currently audited"
              icon="🛡️"
            />
          ...
          <section className="max-w-6xl mx-auto space-y-4">
            <div className="flex justify-between items-center border-b border-slate-800 pb-3">
              <div>
                <h2 className="text-xl font-bold text-white flex items-center gap-2">
                  Tender Accountability Registry
                </h2>
            ...
            {contracts.length > 0 && (
              <div className="text-[10px] text-cyan-400/80 font-medium bg-cyan-950/10 border border-cyan-800/10 px-4 py-2 rounded-lg flex items-center gap-2 max-w-max">
                <span>💡</span>
                <span><b>Interactive Sheet enabled</b>: Click on any tender row to inspect its official <b>"Award of Contract Details"</b>.</span>
              </div>
            )}
            ...
            {loading ? (
              <div className="bg-[#0f172a]/30 border border-slate-800 rounded-xl p-16 text-center space-y-3">
                ...
                <p className="text-slate-400 text-sm">Querying state tender records...</p>
              </div>
            ) : contracts.length === 0 ? (
              <div className="bg-[#0f172a]/30 border border-dashed border-slate-800 rounded-xl p-16 text-center space-y-5 max-w-4xl mx-auto animate-[fadeIn_0.3s_ease-out]">
                ...
                <div className="space-y-2">
                  <h3 className="text-lg font-semibold text-white">No Tenders Found</h3>
                  <p className="text-slate-500 text-xs max-w-lg mx-auto leading-relaxed">
                    No verified contracts match the selected combination of state, department, and filters.
                  </p>
                </div>
              </div>
            ) : (
                  ...
                  <table className="w-full text-left border-collapse text-xs">
                    ...
                    <tbody className="divide-y divide-slate-800/40 text-slate-300">
                      {paginatedContracts.map((c) => {
                        const dateFormatted = c.contractDate ? new Date(c.contractDate).toLocaleDateString("en-IN", {
                          day: '2-digit',
                          month: 'short',
                          year: 'numeric'
                        }) : "N/A";

                        // Set custom badge colors based on category/type
                        const categoryBadgeStyles = 
                          c.category === "NH" ? 'bg-cyan-500/10 text-cyan-400 border border-cyan-500/20' : 
                          c.category === "SH" ? 'bg-yellow-500/10 text-yellow-400 border border-yellow-500/20' : 
                          c.category === "PWD" ? 'bg-green-500/10 text-green-400 border border-green-500/20' : 
                          ...
                          >
                            ...
                            <td className="p-4 font-mono font-bold tracking-wide">
                              <span className="flex items-center gap-2">
                                <span className={`px-1.5 py-0.5 rounded text-[8px] font-bold ${categoryBadgeStyles}`}>
                                  {c.category === "Municipality" ? "MUNI" : c.category}
                                </span>
                                {c.tenderRefNo}
                              </span>
                            </td>
