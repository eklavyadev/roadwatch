...
const IMPACT_DESCRIPTION: Record<Report['type'], Record<number, string>> = {
  ...
  streetlight: {
    ...
    2: 'Often off / unstable',
    3: 'Completely not working',
  },
  traffic_signal: {
    1: 'Delayed / slow response',
...
function StatCard({
  ...
  return (
    <div className="bg-[#0c1525] border border-slate-800 rounded-xl p-4 flex items-center gap-4">
...
export default function AdminPage() {
  ...
  if (!auth) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#020817] px-6">
        {/* Dot grid */}
        <div
        ...
        <div className="relative w-full max-w-sm">
          <div className="bg-[#0c1525] border border-slate-800 rounded-2xl p-8 shadow-2xl shadow-black/50">
            {/* Logo */}
            <div className="flex items-center gap-2.5 mb-8">
              <div className="h-8 w-8 rounded-lg bg-cyan-500 flex items-center justify-center shrink-0">
                <svg width="15" height="15" viewBox="0 0 14 14" fill="none" className="text-[#020817]">
              ...
              <div>
                <p className="text-sm font-semibold text-white">Road<span className="text-cyan-400">Watch</span></p>
            ...
            <div className="space-y-3">
              ...
              <button
                onClick={login}
                className="w-full rounded-lg bg-cyan-500 text-[#020817] py-2.5 text-sm font-semibold hover:bg-cyan-400 active:scale-95 transition-all"
              >
                Sign in
              </button>
            </div>
  ...
  return (
    <div className="min-h-screen bg-[#020817] text-white">
      {/* Header */}
      <div className="border-b border-slate-800 bg-[#020817]/80 backdrop-blur-md sticky top-0 z-10">
        <div className="max-w-7xl mx-auto px-6 py-4 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <div className="h-8 w-8 rounded-lg bg-cyan-500 flex items-center justify-center shrink-0">
              <svg width="14" height="14" viewBox="0 0 14 14" fill="none" className="text-[#020817]">
                <path d="M7 1L13 4V7C13 10.3 10.3 13 7 13C3.7 13 1 10.3 1 7V4L7 1Z" fill="currentColor" fillOpacity="0.3" stroke="currentColor" strokeWidth="1.5" strokeLinejoin="round" />
                <circle cx="7" cy="7.5" r="1.5" fill="currentColor" />
              </svg>
            </div>
            <div>
              ...
              <p className="text-xs text-gray-500">Road<span className="text-cyan-400">Watch</span></p>
            </div>
          </div>
          <button
            onClick={fetchReports}
            className="flex items-center gap-1.5 rounded-lg border border-slate-700 px-3 py-1.5 text-xs text-gray-400 hover:text-white hover:border-slate-600 transition-all"
          >
      ...
      <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all ${
                activeTab === key
                  ...
                  : 'bg-[#0c1525] border border-slate-800 text-gray-400 hover:text-white hover:border-slate-600'
              }`}
            >
              {label}
              {counts[key] > 0 && (
        ...
        {filteredReports.length === 0 ? (
          <div className="border border-dashed border-slate-800 rounded-xl p-16 text-center">
            <p className="text-gray-500 text-sm">No {activeTab} reports</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            {filteredReports.map((r) => (
              <div
                key={r.id}
                className="bg-[#0c1525] border border-slate-800 rounded-xl overflow-hidden hover:border-slate-700 transition-colors"
              >
                {/* Image */}
                  ...
                  {r.status === 'pending' && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-emerald-500/15 border border-emerald-500/25 text-emerald-400 hover:bg-emerald-500/25 py-2 text-sm font-medium transition-all"
                      >
                      ...
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-red-500/15 border border-red-500/25 text-red-400 hover:bg-red-500/25 py-2 text-sm font-medium transition-all"
                      >
                  ...
                  {r.status === 'approved' && (
                    <div className="flex gap-2">
                      <button
                        ...
                        className="flex-1 flex items-center justify-center gap-1.5 rounded-lg bg-slate-700/50 border border-slate-700 text-gray-300 hover:text-white py-2 text-sm font-medium transition-all"
                      >
                  ...
                  {r.status === 'rejected' && (
                    <>
                      ...
                      <button
                        ...
                        className="w-full flex items-center justify-center gap-1.5 rounded-lg border border-red-500/25 text-red-500 hover:bg-red-500/10 py-2 text-sm font-medium transition-all"
                      >
      ...
      {toast && (
        <div
          className={`fixed bottom-6 right-6 flex items-center gap-2.5 rounded-xl border px-4 py-3 text-sm shadow-xl backdrop-blur-sm transition-all ${
            toast.type === 'error'
              ? 'bg-red-500/10 border-red-500/20 text-red-400'
              : 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400'
          }`}
