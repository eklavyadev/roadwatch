...
const RAW_UP_FILE = path.join(__dirname, '..', 'cppp_tenders_full_26_Uttar_Pradesh.json');
const RAW_JHARKHAND_FILE = path.join(__dirname, '..', 'cppp_tenders_full_26_Jharkhand.json');
const RAW_HARYANA_FILE = path.join(__dirname, '..', 'cppp_tenders_full_26_Haryana.json');
...
function runIngestion() {
  ...
  // 1.24. Process Uttar Pradesh
  let upCount = 0;
  if (fs.existsSync(RAW_UP_FILE)) {
    try {
      const rawData = fs.readFileSync(RAW_UP_FILE, 'utf8');
      const parsedData = JSON.parse(rawData);
      if (Array.isArray(parsedData)) {
        console.log(`Found ${parsedData.length} raw tenders in UP.`);
        parsedData.forEach((item, index) => {
          const s = item.structured_data || {};
          const orgName = cleanText(s['Organisation Name'] || '');
          const refNo = cleanText(s['Tender Ref. No.'] || '');
          ...
          if (seenKeys.has(uniqueKey)) { duplicateCount++; } else {
            ...
            contracts.push({
              ...
              tenderDocument: document, tenderType: type, bidsReceived: bids, selectedBidder: bidder,
              contractValue: value, publishedDate: published, contractDate: contractDate, category: 'SH',
              year: year, selectedBidderAddress: address, completionPeriod: completion, state: 'Uttar Pradesh'
            });
            upCount++;
          }
        });
      }
    } catch (e) { console.error('Error reading/parsing UP:', e); }
  } else { console.log(`UP file not found`); }

  // 1.25. Process Jharkhand
  let jharkhandCount = 0;
  if (fs.existsSync(RAW_JHARKHAND_FILE)) {
    try {
      const rawData = fs.readFileSync(RAW_JHARKHAND_FILE, 'utf8');
      const parsedData = JSON.parse(rawData);
      if (Array.isArray(parsedData)) {
        console.log(`Found ${parsedData.length} raw tenders in Jharkhand.`);
        parsedData.forEach((item, index) => {
          const s = item.structured_data || {};
          const orgName = cleanText(s['Organisation Name'] || '');
          const refNo = cleanText(s['Tender Ref. No.'] || '');
          ...
          if (seenKeys.has(uniqueKey)) { duplicateCount++; } else {
            ...
            contracts.push({
              ...
              tenderDocument: document, tenderType: type, bidsReceived: bids, selectedBidder: bidder,
              contractValue: value, publishedDate: published, contractDate: contractDate, category: 'SH',
              year: year, selectedBidderAddress: address, completionPeriod: completion, state: 'Jharkhand'
            });
            jharkhandCount++;
          }
        });
      }
    } catch (e) { console.error('Error reading/parsing Jharkhand:', e); }
  } else { console.log(`Jharkhand file not found`); }

  // 1.26. Process Haryana
  let haryanaCount = 0;
  if (fs.existsSync(RAW_HARYANA_FILE)) {
    try {
      const rawData = fs.readFileSync(RAW_HARYANA_FILE, 'utf8');
      const parsedData = JSON.parse(rawData);
      if (Array.isArray(parsedData)) {
        console.log(`Found ${parsedData.length} raw tenders in Haryana.`);
        parsedData.forEach((item, index) => {
          const s = item.structured_data || {};
          const orgName = cleanText(s['Organisation Name'] || '');
          const refNo = cleanText(s['Tender Ref. No.'] || '');
          ...
          if (seenKeys.has(uniqueKey)) { duplicateCount++; } else {
            ...
            contracts.push({
              ...
              tenderDocument: document, tenderType: type, bidsReceived: bids, selectedBidder: bidder,
              contractValue: value, publishedDate: published, contractDate: contractDate, category: 'SH',
              year: year, selectedBidderAddress: address, completionPeriod: completion, state: 'Haryana'
            });
            haryanaCount++;
          }
        });
      }
    } catch (e) { console.error('Error reading/parsing Haryana:', e); }
  } else { console.log(`Haryana file not found`); }
  ...
  console.log(`Haryana Records: ${haryanaCount}`);
  console.log(`Himachal Pradesh Records: ${himachalCount}`);
  console.log(`Jharkhand Records: ${jharkhandCount}`);
  ...
  console.log(`Uttar Pradesh Records: ${upCount}`);
